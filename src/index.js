...
function findAllPSiblings(where) {
    ...
    for (var i = 0; i < where.childNodes.length; i++) {
        if (where.childNodes[i].tagName == 'P') {
            v.push(where.childNodes[i-1]); // проверить случай когда p - нулевой элемент массива
        }
    }
...
function findError(where) {
    ...
    var inn = where.children;
    var v = [];

    for (var i = 0; i < inn.length; i++) {
        v.push(inn[i].innerText);
    }
...
function deleteTextNodes(where) {
    for (var i = 0; i < where.childNodes.length; i++) {
        if (where.childNodes[i].nodeType == document.TEXT_NODE) {
...
function deleteTextNodesRecursive(where) {
    ...
    for (var i = 0; i < child.length; i++) {
        if (child[i].nodeType == document.ELEMENT_NODE) {
            deleteTextNodesRecursive(child[i]);
        } else if (child[i].nodeType == document.TEXT_NODE) {
